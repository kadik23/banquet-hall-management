...
function MainReservation({ searchTerm }: { searchTerm: string }) {
  ...
  useEffect(() => {
    setFilteredReservations(
      reservations.filter((reservation) => {
        // Formatage des dates en mode européen
        const formattedStartDate = reservation.start_date
          ? format(new Date(reservation.start_date), 'dd-MM-yyyy' , { locale: fr })
          : '';
        const formattedReservationDate = reservation.date_reservation
          ? format(new Date(reservation.date_reservation), 'dd-MM-yyyy' , { locale: fr })
          : '';
  
        // Création de la chaîne à chercher
        const searchableString = [
          reservation.client_id,
          formattedReservationDate,
          formattedStartDate,
          reservation.period,
          reservation.nbr_invites,
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (isEditMode) {
      ...
      window.alert(`La réservation ${newReservation.id} a été modifiée avec succès`);
      ...
      window.alert(`La réservation ${data.reservationId} a été créée avec succès`);
      ...
      <table className="reservations-table">
        <thead>
          <tr>
            ...
            <th>date de début du marriage </th>
